        ...
        <v-col cols="12">
          <v-card
                ...
                <v-card-title>
                  ...
                  >Add your daily activity in Blog</h1>
                </v-card-title>
                <v-card-actions>
                     <v-list-item class="grow">
                        <v-row
                          ...
                          >
                          <v-col>
                          <router-link :to="{ name: 'addBlog'}">Add New blog</router-link>
                          <!-- <v-btn
          ...
          <v-col cols="12">
            <!-- <v-btn>show blog</v-btn> -->
              <v-card
                v-for="(blog ,index ) in blogdummy" :key="index"
                class="mx-auto mt-10"
                color="#8FBC8F"
                dark
                max-width="75%">
                <v-card-title>
                    <v-icon
                       large
                       left>mdi-comment-text-outline</v-icon>
                    <v-text-field
                    disabled
                     v-model="blog.name" class="text-h6 font-weight-light">{{blog.name}}</v-text-field>
                </v-card-title>
                 ...
                 disabled
                   ...
                   label="Your Blog"
                   rows="4"
                   prepend-icon="mdi-comment">{{blog.blogtext}}</v-textarea>
                   <v-file-input
                   disabled
                     ...
                     <v-list-item class="grow">
                        <v-row
                          ...
                          <div class="action-button">
                           ...
                           <v-btn 
                           class="mr-5 py-5"
                           @click="deletePost(blog.id,index)" small>
                           <v-icon @click="deletePop = true" class="mr-1">mdi-delete</v-icon>
                           Delete Blog
                           </v-btn>
            ...
            <v-col cols="12">
              <v-snackbar
                 v-model="deletePop">Blog deleted successfully
                 <template v-slot:action="{ attrs }">
                    <v-btn 
                      color="pink"
                      text
                      v-bind="attrs"
                      @click="deletePop = false">close Notification</v-btn>
                  </template>
               </v-snackbar>
            </v-col>
      </v-row>
